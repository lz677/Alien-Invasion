...
class Scoreboard:
    def __init__(self, ai_setting, screen, stats):
        # initialize the score and its attributes
        self.screen = screen
        self.screen_rect = self.screen.get_rect()
        self.ai_setting = ai_setting
        self.stats = stats

        # the score's font settings
        self.text_color = (30, 30, 30)
        self.font = pygame.sysfont.SysFont(None, 48)

        # prepare the initialized score image
        self.prep_score()
    ...
    def prep_score(self):
        # change the score to a image
        rounded_score = int(round(self.stats.score, -1))
        score_str = "{:,}".format(rounded_score)
        self.score_image = self.font.render(score_str, True, self.text_color,
                                            self.ai_setting.bg_color)

        # put score on the right of top
